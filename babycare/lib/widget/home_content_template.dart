...
class _HomeContentTemplateState extends State<HomeContentTemplate> {
    ...
    return Card(
      child: new Container(
        child: BaseListContent(
            GlobalConfig.getModelIconPath(_title), createContentWidget("刚刚", "测试100毫升")),
      ),
    );
  }
  ...
  createContentWidget(String recordTime, String contentText) {
    ...
    // 历史记录时间
    contents.add(new Text(recordTime,
        style: new TextStyle(color: GlobalConfig.getModelColor(_title), fontSize: 16), textAlign: TextAlign.left,));

    // 历史记录内容
    contents.add(new Text(contentText,
        style: new TextStyle(color: GlobalConfig.getModelColor(_title), fontSize: 18), textAlign: TextAlign.left,));
